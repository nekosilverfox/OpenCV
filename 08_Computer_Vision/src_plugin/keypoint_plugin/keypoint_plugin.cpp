...
void Keypoint_Plugin::processImage(const cv::Mat &inputImage, cv::Mat &outputImage)
{
    ...
    descMather = DescriptorMatcher::create(static_cast<DescriptorMatcher::MatcherType>(ui->matcherCombo->currentIndex() + 1));
...
void Keypoint_Plugin::fillFeature2D(QString algName, cv::Ptr<cv::Feature2D> &algorithm)
{
    ...
    if(algName == "AGAST")
    {
        ...
        agast->setType(static_cast<AgastFeatureDetector::DetectorType>(ui->agastTypeCombo->currentIndex()));
        algorithm = agast;
    }
    else if(algName == "KAZE")
    {
        if(ui->kazeAcceleratedCheck->isChecked())
        {
            ...
            akaze->setDescriptorType(static_cast<AKAZE::DescriptorType>(ui->akazeDescriptCombo->currentIndex() + 2));
            akaze->setDiffusivity(static_cast<KAZE::DiffusivityType>(ui->kazeDiffCombo->currentIndex()));
        ...
        else
        {
            ...
            kaze->setDiffusivity(static_cast<KAZE::DiffusivityType>(ui->kazeDiffCombo->currentIndex()));
    ...
    else if(algName == "FAST")
    {
        ...
        fast->setType(static_cast<FastFeatureDetector::DetectorType>(ui->fastTypeCombo->currentIndex()));
        algorithm = fast;
    }
